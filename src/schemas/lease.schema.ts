...
export const CreateLeaseSchema = z.object({
    unitId: z.string({ required_error: 'Unit ID is required' }).cuid2({ message: 'Invalid Unit ID format' }),
    tenantId: z.string({ required_error: 'Tenant ID is required' }).cuid2({ message: 'Invalid Tenant ID format' }),
    startedAt: z.coerce.date({ // coerce attempts to convert string/number to Date
