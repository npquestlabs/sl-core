...
//  * Records a new payment for a lease owned by the authenticated staff.
...
//   }
//   if (!user.staff) {
//     throw new AppError('User does not have staff privileges', 403)
//   }

//   const input = req.body as CreatePaymentInput

//   try {
//     const newPayment = await createPayment(user.staff.id, input)
...
//  * Lists payments for a property (complex) owned by the authenticated staff.
...
//   if (user.staff) {
//     whereClause.lease.staffId = user.staff.id
//   } else if (user.tenant) {
...
//   const payment = getPayment(whereClause, {
//     lease: { include: { tenant: true, staff: true } },
//   })
