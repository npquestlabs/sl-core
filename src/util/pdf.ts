...
const getText = (value: string | null | undefined, prefix = '') =>
  prefix + (value || 'N/A')
 ...
 */
const savePDFAndUploadToCloudinary = async (
  ...
  try {
    ...
    const result = await cloudinary.uploader.upload(outputPath, {
      resource_type: 'raw',
      public_id: `smart-staff/documents/${filename.replace('.pdf', '')}`,
      overwrite: true,
    })
    ...
    if (fs.existsSync(outputPath)) {
      try {
        ...
        logger.error(
          `Failed to delete temporary file ${outputPath} after upload error:`,
          unlinkErr,
        )
      }
    }
    ...
    throw new Error(
      `Failed to upload PDF to Cloudinary. ${error.message || error}`,
    )
  }
}
...
const addWatermark = (doc: typeof PDFDocument) => {
  const watermarkText = 'Smart Staff Platform'
...
export const generateLeasePDF = async (
  ...
  doc.fontSize(14).text('Staff Details', { underline: true })
  doc.fontSize(12)
  if (lease.staff?.user) {
    doc.text(
      getText(`${lease.staff.firstName} ${lease.staff.lastName}`, 'Name: '),
    )
    doc.text(getText(lease.staff.user.email, 'Email: '))
    doc.text(getText(lease.staff.phone, 'Phone: '))
  } else {
    doc.text('Staff details not available.')
  }
  doc.moveDown()
  ...
  if (lease.tenant?.user) {
    doc.text(
      getText(`${lease.tenant.firstName} ${lease.tenant.lastName}`, 'Name: '),
    )
  ...
  doc.text(
    `Advance Duration Paid: ${formatAdvanceDuration(lease.advanceSeconds)}`,
  )
  ...
  doc.moveDown(2)
  doc.fontSize(12)
  doc.text('Staff: ___________________________')
  doc.moveDown(1.5)
  ...
  doc
    .fontSize(8)
    .text(
      `Document generated by Smart Staff Platform on ${today.toISOString()}`,
      { align: 'center' },
    )
 ...
 * @returns {Promise<string>} The secure URL of the uploaded PDF on Cloudinary.
 */
export const generateRentCardPDF = async (
  lease: RentCardPayload,
): Promise<string> => {
  const doc = new PDFDocument({ margin: 50, bufferPages: true })
  ...
  doc
    .fontSize(10)
    .text(
      'Issued in compliance with the Rent Act, 1963 (Act 220) and Rent Control Law, 1986 (PNDCL 138) of Ghana.',
      {
        align: 'center',
        lineGap: 3,
      },
  ...
  doc
    .fontSize(8)
    .text(
      `Document generated by Smart Staff Platform on ${new Date().toISOString()}`,
      { align: 'center' },
    )
...
export const generateReceiptPDF = async (
  ...
  doc.fontSize(10).text(`Transaction Ref: ${payment.transactionRef || 'N/A'}`, {
    align: 'center',
  })
  doc.moveDown(2)

  doc.fontSize(12)
  const tenantName = payment.lease?.tenant?.user
  ...
  if (payment.paymentStatus === PaymentStatus.COMPLETED) {
    ...
    doc
      .fontSize(16)
      .fillColor('red')
      .text(' PAYMENT FAILED', { align: 'center' })
  } else {
    doc
      .fontSize(12)
      .fillColor('black')
      .text(`Status: ${payment.paymentStatus || 'UNKNOWN'}`, {
        align: 'center',
      })
  }
  ...
  doc
    .fontSize(10)
    .text(`Receipt Generated: ${new Date().toLocaleString('en-GB')}`, {
      align: 'center',
    })
  doc.moveDown()
  doc
    .fontSize(8)
    .text(`Document generated by Smart Staff Platform`, { align: 'center' })
