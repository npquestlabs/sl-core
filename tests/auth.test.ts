...
const generateUserRegistrationData = (
    ...
    if (type === 'vendor') return { ...baseData, vendor: { specialty: faker.commerce.department() } };
...
const setupVerifiedUser = async (
    ...
    if (registerResponse.status !== 201 && registerResponse.status !== 202) {
        console.error('Failed to register user in setupVerifiedUser:', registerResponse.body);
        throw new Error(`Setup: Registration failed with status ${registerResponse.status} and body: ${JSON.stringify(registerResponse.body)}`);
    }
    const { emailToken: verificationToken } = registerResponse.body as RegisterApiResponse;
    if (!verificationToken) {
        throw new Error('Setup: No emailToken returned from registration. This token is expected to contain all necessary data for user creation (including hashed password).');
    }
    ...
    if (verifyResponse.status !== 200 && verifyResponse.status !== 201) {
        console.error('Failed to verify user in setupVerifiedUser:', verifyResponse.body);
        throw new Error(`Setup: Verification failed with status ${verifyResponse.status} and body: ${JSON.stringify(verifyResponse.body)}`);
    }
    ...
    if (!createdUserFromApi || !accessToken) {
        throw new Error('Setup: User data or access token not found in verification response');
    }
    ...
    if (!dbUser) throw new Error("Setup: User not found in DB after verification, though API indicated success.");

    return { user: dbUser as LocalUser, accessToken, rawPassword: registrationData.password! };
};
...
afterAll(async () => {
    ...
    if (userIdsToDelete.length > 0) {
        await prisma.landlord.deleteMany({ where: { user: { id: { in: userIdsToDelete } } } });
        await prisma.tenant.deleteMany({ where: { user: { id: { in: userIdsToDelete } } } });
        await prisma.vendor.deleteMany({ where: { user: { id: { in: userIdsToDelete } } } });
...
afterEach(async () => {
    if (testCreatedUserIds.length > 0) {
        await prisma.landlord.deleteMany({ where: { user: { id: { in: testCreatedUserIds } } } });
        await prisma.tenant.deleteMany({ where: { user: { id: { in: testCreatedUserIds } } } });
        await prisma.vendor.deleteMany({ where: { user: { id: { in: testCreatedUserIds } } } });
...
describe('Auth Routes', () => {
    describe('POST /auth/register & POST /auth/verify', () => {
        it('should register (send token with embedded user data), then verify (create user from token), and allow login', async () => {
            ...
            const dbUserBeforeVerify = await prisma.user.findUnique({ where: { email: landlordRegData.email } });
