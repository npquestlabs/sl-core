...
afterEach(async () => {
    ...
    if (testCreatedComplexIds.length > 0) {
        await prisma.complex.deleteMany({ where: { id: { in: testCreatedComplexIds } } });
    }
...
describe('Complex Routes', () => {
    describe('POST /complexes', () => {
        it('should create a new complex for the authenticated landlord', async () => {
            ...
            const dbComplex = await prisma.complex.findUnique({ where: { id: response.body.id } });
    ...
    describe('GET /complexes', () => {
        ...
        it('should get a list of complexes specifically created for the authenticated landlord', async () => {
            ...
            const createdComplex = await createComplexForTestAndTrack({ ...complexData, landlordId: landlord.id });
            ...
            await prisma.complex.create({
                data: { ...generateComplexData("_other_landlord_temp"), landlordId: otherLandlord.id },
            });
        ...
        it('should support pagination (page and limit)', async () => {
            ...
            await prisma.complex.create({ data: { ...generateComplexData('_pg_other'), landlordId: otherLandlord.id } });
        ...
        it('should support filtering by name', async () => {
            ...
            const targetComplex = await createComplexForTestAndTrack({
                ...generateComplexData(), name: uniqueName, landlordId: landlord.id
            });
            await createComplexForTestAndTrack({ // Another complex for the same landlord, won't match filter
                ...generateComplexData('_filter_nonmatch'), landlordId: landlord.id
            });
    ...
    describe('GET /complexes/:complexId', () => {
        ...
        beforeEach(async () => { // This runs AFTER global beforeEach
            complexOfLandlord = await createComplexForTestAndTrack({
                ...generateComplexData('_get_by_id'), landlordId: landlord.id
            });
            ...
            complexOfOtherRunLandlord = await prisma.complex.create({
                data: { ...generateComplexData('_get_by_id_other'), landlordId: otherLandlord.id },
            });
        });
        ...
        it('should return 401 if not authenticated', async () => {
            const response = await request(app).get(`/api/v1/complexes/${complexOfLandlord.id}`);
            expect(response.status).toBe(401);
        });
    });
    ...
    describe('PATCH /complexes/:complexId', () => {
        ...
        beforeEach(async () => {
            complexToUpdate = await createComplexForTestAndTrack({
                ...generateComplexData('_patch_target'), landlordId: landlord.id
            });
        });
        ...
        it('should return 404 if complex belongs to another landlord (of this run)', async () => {
            const otherComplex = await prisma.complex.create({ // Belongs to otherLandlord of this run
                data: { ...generateComplexData('_patch_other'), landlordId: otherLandlord.id },
            });
    ...
    describe('DELETE /complexes/:complexId', () => {
        ...
        beforeEach(async () => {
            complexToDelete = await createComplexForTestAndTrack({
                ...generateComplexData('_delete_target'), landlordId: landlord.id
            });
        });
        ...
        it('should return 404 if complex belongs to another landlord (of this run)', async () => {
            const otherComplex = await prisma.complex.create({ // Belongs to otherLandlord of this run
                data: { ...generateComplexData('_delete_other'), landlordId: otherLandlord.id },
            });
            // This otherComplex will be cleaned by afterAll.
            const response = await request(app)
